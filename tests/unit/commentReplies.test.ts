...
describe('comment reply helpers', () => {
  it('flattens nested recent replies and infers parent ids from the API tree', () => {
    const normalized = normalizeCommentReplyTree(
      comment({
        id: 'root',
        recent_replies: [
          comment({
    ...
    expect(normalized.recent_replies?.map((reply) => reply.id)).toEqual(['reply-1', 'nested-reply-1', 'nested-reply-2'])
    expect(normalized.recent_replies?.map((reply) => reply.parent_comment_id)).toEqual(['root', 'reply-1', 'reply-1'])
