...
describe('eventBookmark', () => {
  ...
  it('refreshes bookmark state on mount by default', async () => {
    ...
    render(<EventBookmark event={createEvent()} />)
  ...
  it('skips the mount refresh when disabled by list cards', async () => {
    render(<EventBookmark event={createEvent()} refreshStatusOnMount={false} />)
  ...
  it('only rewrites the acting user event caches', async () => {
    ...
    const homeResolvedBookmarkedUserKey = [
      'events',
      'trending',
      'trending',
      '',
    ...
    const cachedQueries = [
      [
        homeResolvedUserKey,
        { pages: [{ events: [{ id: 'event-1', is_bookmarked: false }], hasMore: false }], pageParams: [0] },
      ],
      [
      ...
      [
        homeResolvedGuestKey,
        { pages: [{ events: [{ id: 'event-1', is_bookmarked: false }], hasMore: false }], pageParams: [0] },
      ],
      [sportsUserKey, { pages: [[{ id: 'event-1', is_bookmarked: false }]], pageParams: [0] }],
    ] as const

    mocks.getQueriesData.mockImplementation(
      ({ predicate }: { predicate?: (query: { queryKey: readonly unknown[] }) => boolean }) =>
  ...
  it('preserves bookmarked-only home page boundaries when unbookmarking', async () => {
    ...
    const cachedQueries = [
      [
        homeResolvedUserKey,
        { pages: [{ events: [{ id: 'event-1', is_bookmarked: true }], hasMore: true }], pageParams: [0] },
      ],
      [
        homeResolvedBookmarkedUserKey,
        { pages: [{ events: [{ id: 'event-1', is_bookmarked: true }], hasMore: true }], pageParams: [0] },
    ...
    render(<EventBookmark event={createEvent({ is_bookmarked: true })} refreshStatusOnMount={false} />)
