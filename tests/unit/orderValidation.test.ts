...
describe('validateOrder', () => {
  ...
  it('validates limit order price/shares and min shares', () => {
    expect(validateOrder(baseArgs({ isLimitOrder: true, limitPrice: '0', limitShares: '10' }))).toEqual({
      ok: false,
      reason: 'INVALID_LIMIT_PRICE',
    })
    expect(validateOrder(baseArgs({ isLimitOrder: true, limitPrice: '50', limitShares: '0' }))).toEqual({
      ok: false,
      reason: 'INVALID_LIMIT_SHARES',
    })
