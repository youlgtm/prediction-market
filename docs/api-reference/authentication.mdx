...
<Callout type="info">
  L2 authenticates the request. A new order still contains an order payload signed by the
  trader before submission.
</Callout>
...
<Callout type="warning" title="Store credentials as secrets">
  Keep the private key, API secret, and passphrase in a server-side secret store. Do not
  place them in frontend bundles, logs, analytics, URLs, or support messages.
</Callout>
