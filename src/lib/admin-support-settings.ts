import { DEFAULT_THEME_SITE_LOGO_SVG, DEFAULT_THEME_SITE_NAME } from '@/lib/theme-site-identity'
...
export function isAdminOnboardingTaskId(value: unknown): value is AdminOnboardingTaskId {
  return typeof value === 'string' && (ADMIN_ONBOARDING_TASK_IDS as readonly string[]).includes(value)
}
...
export function getCompletedAdminOnboardingTasks(settings?: SettingsMap | null) {
  ...
  const hasCustomLogo = Boolean(logoImagePath || (logoSvg && logoSvg !== DEFAULT_THEME_SITE_LOGO_SVG))
...
export function getSupportAnnouncementDismissedAt(settings?: SettingsMap | null) {
  const rawValue = settings?.[ADMIN_SUPPORT_SETTINGS_GROUP]?.[ADMIN_SUPPORT_ANNOUNCEMENT_DISMISSED_AT_KEY]?.value.trim()
