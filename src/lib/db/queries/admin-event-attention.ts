...
export function buildMissingSportsSourceCondition() {
  const hasSportsTag = exists(
    db
      ...
      .where(and(eq(event_tags.event_id, events.id), eq(tags.slug, 'sports'))),
  )
    ...
    db
      ...
      .where(
        and(
          ...
          or(
            and(hasEsportsTag, eq(event_sports.sports_source_provider, 'pandascore')),
            and(not(hasEsportsTag), hasSportsTag, eq(event_sports.sports_source_provider, 'thesportsdb')),
          ),
          sql`(
