...
} from '@/lib/db/schema/sumsub/tables'
...
export const SumsubRepository = {
  ...
  async getStatusesForUsers(userIds: string[], levelName: string) {
    ...
    const rows = await db
      ...
      .where(and(inArray(sumsub_applicants.user_id, userIds), eq(sumsub_applicants.level_name, levelName)))
    return new Map(rows.map((row) => [row.userId, row.status as SumsubStatus]))
  },

  async consumeRateLimit(
    userId: string,
    scope: 'access_token' | 'status' | 'test_connection',
    limit: number,
    windowSeconds = 60,
  ) {
    const [row] = await db
      .insert(sumsub_access_token_rate_limits)
      .values({
        user_id: userId,
        scope,
        request_count: 1,
  ...
  async processWebhook(fingerprint: string, update: SumsubApplicantUpdate, eventType: string) {
    return db.transaction(async (tx) => {
      const inserted = await tx
        ...
        .onConflictDoNothing()
        .returning({ fingerprint: sumsub_webhook_events.fingerprint })
      ...
      if (
        associated.lastEventCreatedAt &&
        update.eventCreatedAt &&
        associated.lastEventCreatedAt >= update.eventCreatedAt
      ) {
