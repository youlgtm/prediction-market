...
function haveSameSeriesKeys(a: DataPoint, b: DataPoint) {
  ...
  return aKeys.length === bKeys.length && aKeys.every((key) => Object.hasOwn(b, key))
}
...
function usePredictionChartData(
  ...
  useLayoutEffect(
    function cleanupScheduledStateUpdate() {
    ...
    function syncProvidedData() {
      ...
      setData((previousData) => {
        ...
        if (dataSyncMode === 'replace') {
          const dataMatchesExactly =
            previousData.length === providedData.length &&
        ...
        const hasInvalidTimeline = timelineValues.some((value) => typeof value !== 'number' || !Number.isFinite(value))
        ...
        if (typeof incomingLast === 'number' && typeof previousLast === 'number' && incomingLast < previousLast) {
          lastDataUpdateTypeRef.current = 'reset'
          return providedData
        }

        if (typeof incomingFirst === 'number' && typeof previousFirst === 'number' && incomingFirst < previousFirst) {
          lastDataUpdateTypeRef.current = 'reset'
          return providedData
        }
        ...
        const lastTimestamp = latestNextPoint ? latestNextPoint.date.getTime() : null
