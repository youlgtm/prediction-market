...
export function useInfiniteLoadMore({
  ...
  const scopedLoadMoreState =
    loadMoreState.key === loadMoreScopeKey
      ? loadMoreState
      : {
          key: loadMoreScopeKey,
          infiniteScrollError: null,
  ...
  const loadMore = useCallback(
    function loadMore() {
      if (!hasNextPage || isFetchingNextPage || scopedLoadMoreState.isLoadingMore || isLoadMoreInFlightRef.current) {
        return
      }
