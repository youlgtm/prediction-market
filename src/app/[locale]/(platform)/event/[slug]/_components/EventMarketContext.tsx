...
function ActiveEventMarketContext({ event, marketConditionId }: EventMarketContextProps) {
  ...
  return (
    <EventMarketContextContent key={contextKey} event={event} resolvedMarketConditionId={resolvedMarketConditionId} />
  )
}
...
function useMarketContextState(event: Event, resolvedMarketConditionId: string | undefined) {
  ...
  useEffect(
    function preloadCachedContextEffect() {
  ...
  useEffect(
    function cacheExpirationEffect() {
      if (!cacheExpiresAtMs) {
        return
      }
  ...
  useEffect(
    function typewriterAnimationEffect() {
      let isActive = true
...
function EventMarketContextContent({ event, resolvedMarketConditionId }: EventMarketContextContentProps) {
  ...
  return (
    <section className="overflow-hidden rounded-xl border transition-all duration-500 ease-in-out">
      ...
      >
        <div className={cn('min-h-0 overflow-hidden', { 'border-t border-border/30': isContentExpanded })}>
          <div className="space-y-3 p-3">
            ...
            {!error && context && !isTyping && displayedContext === context && (
              <div className="flex justify-end">
                <span className="font-mono text-2xs tracking-wide text-muted-foreground/80">{updatedLabel}</span>
              </div>
            )}
          </div>
        </div>
