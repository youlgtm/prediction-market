...
function SortHeaderButton({ label, sortKey, sortBy, sortDirection, onSortHeaderClick }: SortHeaderButtonProps) {
  ...
  return (
    <button
      ...
      className={cn(
        `inline-flex items-center gap-1 rounded-md px-2 py-1 tracking-wide whitespace-nowrap uppercase transition-colors hover:bg-muted/70 hover:shadow-sm`,
          ...
          <>
            <tbody className="divide-y divide-border/60">
              {positions.map((position) =>
                isClosed ? (
                  <PublicClosedPositionsRow key={position.id} position={position} onShareClick={onShareClick} />
                ) : (
                  <PublicPositionsRow
                    key={position.id}
                    position={position}
            ...
            {!isClosed && (
              <tfoot>
                <tr className="border-t text-sm font-semibold">
                  ...
                  <td className="px-2 py-3 text-center tabular-nums sm:px-3">{formatCurrencyValue(totals.trade)}</td>
                  <td className="px-2 py-3 text-center tabular-nums sm:px-3">{formatCurrencyValue(totals.toWin)}</td>
