...
import {
  deleteAccountAction,
  deleteRelayerUserDataAction,
} from '@/app/[locale]/(platform)/settings/_actions/delete-account'
...
import { deleteCommunityProfileData, requestCommunityProfileDeleteNonce } from '@/lib/community-profile'
...
export default function SettingsDeleteAccountContent({ user }: { user: User }) {
  ...
  const linkedWalletAddress =
    ...
    normalizedConnectedAddress.toLowerCase() === normalizedUserAddress.toLowerCase()
      ? normalizedConnectedAddress
      : null
  ...
  const deleteCommunityData = useCallback(
    async (address: `0x${string}`) => {
      const token = await ensureCommunityToken({
        address,
        signMessageAsync: (args) =>
          runWithSignaturePrompt(() => signMessageAsync(args), {
      ...
      const signature = await runWithSignaturePrompt(() => signMessageAsync({ message: noncePayload.message }), {
        title: t('Delete account'),
        description: t('Open your wallet and approve the signature to continue.'),
      })
  ...
  const deleteRelayerData = useCallback(
    async (address: `0x${string}`) => {
  ...
  useEffect(
    function resumeDeleteAfterWalletConnection() {
      const resumedWalletAddress =
        shouldResumeDeleteAfterWalletConnection && isDialogOpen && isDeleteConfirmed && !isPending
          ? linkedWalletAddress
          : null
