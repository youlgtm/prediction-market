...
export default function EventCardMarketsList({
  ...
  const marketsToRender = isResolvedEvent
    ? markets
        .map((market, index) => {
          ...
          const rank =
            resolvedOutcomeIndex === OUTCOME_INDEX.YES ? 0 : resolvedOutcomeIndex === OUTCOME_INDEX.NO ? 1 : 2
  ...
  return (
    <div className={cn('max-h-16 space-y-2 overflow-y-auto', isResolvedEvent ? 'mb-1' : 'mb-2')}>
      {marketsToRender.map((market) => {
        const resolvedOutcomeIndex = isResolvedEvent
          ...
          : null
        const resolvedOutcome = isResolvedEvent ? resolveBinaryOutcomeByIndex(market, resolvedOutcomeIndex) : null
            ...
            <div className="ml-2 flex items-center gap-2">
              {isResolvedEvent ? (
                resolvedOutcome ? (
                  <span
                    className={cn(
                      `inline-flex items-center gap-2 rounded-md px-2.5 py-1 text-sm font-semibold text-foreground transition-colors group-hover:bg-card`,
                    )}
                  >
                    <span
                      className={cn(
                  ...
                  <span
                    className={cn(
                      `inline-flex items-center rounded-md px-2.5 py-1 text-sm font-semibold text-muted-foreground transition-colors group-hover:bg-card`,
                    )}
                  >
                    {t('Resolved')}
                  </span>
