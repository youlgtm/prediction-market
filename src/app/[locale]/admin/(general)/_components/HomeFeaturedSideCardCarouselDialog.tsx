...
export default function HomeFeaturedSideCardCarouselDialog({
  ...
  const selectedIndex = Math.max(
    0,
    sideCard.slides.findIndex((slide) => slide.id === selectedSlideId),
  )
  ...
  function updateSelectedSlide(updates: Partial<HomeFeaturedSideCardSlide>) {
    replaceSlides((slides) =>
      slides.map((slide) => (slide.id === selectedSlide?.id ? { ...slide, ...updates } : slide)),
    )
  }
        ...
        <div className="grid gap-2">
          {sideCard.slides.map((slide, index) => (
            ...
            >
              ...
              <span
                className={cn('size-2 rounded-full', slide.enabled ? 'bg-emerald-500' : 'bg-muted-foreground/30')}
              />
            </button>
          ))}
        </div>
        ...
        <div className="grid content-start gap-5 rounded-xl border p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <label className="flex items-center gap-2 text-sm font-medium">
              <Switch
                ...
                onCheckedChange={(enabled) => updateSelectedSlide({ enabled })}
                disabled={disabled}
              />
              {t('Active slide')}
            </label>
            <div className="flex items-center gap-1">
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                disabled={disabled || selectedIndex === sideCard.slides.length - 1}
                onClick={() => moveSelectedSlide(1)}
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                disabled={disabled || sideCard.slides.length <= 1}
            ...
            <div className="grid gap-3">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <label
                  ...
                  className={cn(
                    `inline-flex h-9 cursor-pointer items-center rounded-md border bg-background px-4 text-sm font-medium`,
          ...
          {selectedSlide.type === 'video' && (
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(event) =>
                  updateSelectedSlide({
                    videoUrl: event.target.value.slice(0, HOME_FEATURED_SIDE_CARD_LIMITS.videoUrl),
                  })
                }
              ...
              />
              <p className="text-xs text-muted-foreground">
                {t('Paste a public video URL. It will be converted to a privacy-safe embed when saved.')}
              </p>
            </div>
          )}
          ...
          {selectedSlide.type === 'text' && (
            <>
              <div className="grid gap-2">
                ...
                <Input
                  id={`home-featured-side-title-${selectedSlide.id}`}
                  value={selectedSlide.title}
                  onChange={(event) =>
                    updateSelectedSlide({ title: event.target.value.slice(0, HOME_FEATURED_SIDE_CARD_LIMITS.title) })
                  }
              ...
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  onChange={(event) =>
                    updateSelectedSlide({ text: event.target.value.slice(0, HOME_FEATURED_SIDE_CARD_LIMITS.text) })
                  }
              ...
              <label className="flex items-center justify-between gap-4 rounded-lg border p-3">
                ...
                <Switch
                  ...
                  onCheckedChange={(useAi) => updateSelectedSlide({ useAi })}
                  disabled={disabled}
                />
              </label>
            </>
          ...
          {selectedSlide.type !== 'video' && (
            <div className="grid gap-4 md:grid-cols-2">
              <div className="grid gap-2">
                <Label htmlFor={`home-featured-side-cta-label-${selectedSlide.id}`}>
                  {selectedSlide.type === 'image' ? t('Hover text') : t('CTA label')}
                </Label>
                <Input
                  id={`home-featured-side-cta-label-${selectedSlide.id}`}
                  value={selectedSlide.ctaLabel}
                  onChange={(event) =>
                    updateSelectedSlide({
                      ctaLabel: event.target.value.slice(0, HOME_FEATURED_SIDE_CARD_LIMITS.ctaLabel),
                    })
                  }
              ...
              <div className="grid gap-2">
                ...
                <Input
                  id={`home-featured-side-cta-link-${selectedSlide.id}`}
                  value={selectedSlide.ctaHref}
                  onChange={(event) =>
                    updateSelectedSlide({
                      ctaHref: event.target.value.slice(0, HOME_FEATURED_SIDE_CARD_LIMITS.ctaHref),
                    })
                  }
      ...
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="max-h-[92dvh]">
          <DrawerHeader className="text-left">
            ...
            <DrawerDescription>
              {t('Add text, image, or video slides. Active slides rotate automatically and pause on hover.')}
            </DrawerDescription>
          </DrawerHeader>
