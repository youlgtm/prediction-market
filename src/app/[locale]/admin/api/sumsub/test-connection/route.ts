...
export async function POST(request: Request) {
  ...
  try {
    ...
    const appToken =
      typeof input.appToken === 'string' && input.appToken.trim() ? input.appToken.trim() : stored.appToken
    const secretKey =
      typeof input.secretKey === 'string' && input.secretKey.trim() ? input.secretKey.trim() : stored.secretKey
    ...
    return NextResponse.json(
      { error: error instanceof SumsubClientError ? error.message : 'Unable to test Sumsub.' },
      { status },
    )
  }
}
