...
function useDataTableState<TData extends RowData>({
  ...
  const sorting: SortingState = useMemo(() => {
    ...
    const hasMappedColumn = columns.some((column) => {
      ...
      const accessorKey = 'accessorKey' in column && column.accessorKey != null ? String(column.accessorKey) : null
          ...
          <div className="flex flex-col items-center justify-center px-4 py-12 text-center">
            ...
            {onRetry && (
              <button
                ...
                onClick={onRetry}
                className={cn(
                  `inline-flex items-center rounded-md border border-transparent bg-primary px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-primary/90 focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:outline-none`,
                )}
              >
                {t('Try again')}
              </button>
                ...
                {headerGroup.headers.map((header) => {
                  return (
                    <TableHead
                      ...
                      className={cn(tableHeaderClass, 'px-1 sm:px-2')}
                    >
