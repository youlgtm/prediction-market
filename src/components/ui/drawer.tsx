    ...
    <DrawerPortal>
      ...
      >
        <DrawerPrimitive.Popup
          ...
          className={cn(
            'group/drawer-popup pointer-events-auto fixed z-50 m-(--drawer-inset,0px) flex h-(--drawer-content-height) max-h-(--drawer-content-max-height,none) min-h-0 w-(--drawer-content-width,auto) transform-[translate3d(var(--translate-x,0px),var(--translate-y,0px),0)_scale(var(--stack-scale))] flex-col bg-background text-sm transition-[transform,height,opacity,filter] duration-450 ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform outline-none select-none [interpolate-size:allow-keywords] data-[swipe-direction=down]:rounded-t-[10px] data-[swipe-direction=down]:border-t data-[swipe-direction=left]:rounded-r-[10px] data-[swipe-direction=left]:border-r data-[swipe-direction=right]:rounded-l-[10px] data-[swipe-direction=right]:border-l data-[swipe-direction=up]:rounded-b-[10px] data-[swipe-direction=up]:border-b',
...
function DrawerHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('mt-4 grid shrink-0 gap-1.5 p-4 text-center sm:text-left', className)}
      {...props}
    />
  )
}
